############################# computation of VaR using CVine copula
rm(list = ls(all = TRUE))
graphics.off()
...
libraries = c("CDVine", "fGarch", "foreach", "doParallel", "copula")
...
objFun = function(dMat) {
    library(CDVine)
    info      = CDVineCopSelect(dMat, type = 1)
    para.vine = c(info$family, info$par, info$par2)
    return(para.vine)
}
...
objFunVaR   = function(dMat) {
    i = dMat
    library(CDVine)
    para.EmployedVine = para.vec[i, ]
    u                 = CDVineSim(M, para.EmployedVine[1:10], para.EmployedVine[11:20], 
                                 para.EmployedVine[21:30], type = 1)
    u                 = qnorm(u) 
