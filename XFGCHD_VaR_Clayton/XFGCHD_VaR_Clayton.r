# computation of VaR using Clayton copula
rm(list = ls(all = TRUE))
graphics.off()
...
objFun = function(dMat) {
    library(copula)
    clayton.cop = claytonCopula(2, dim = dims)
    fit.mln     = fitCopula(clayton.cop, dMat, method = "mpl")
...
objFunVaR   = function(dMat) {
    ...
    clayton.cop.2    = claytonCopula(para.normal, dim = dims)
    u                = rCopula(M, clayton.cop.2)
    u                = qnorm(u)
